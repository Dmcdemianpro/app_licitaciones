    ...
    <div className="flex min-h-screen flex-col bg-gradient-to-b from-slate-900 via-slate-950 to-slate-900 text-slate-50">
      <header className="flex items-center justify-between border-b border-white/10 bg-white/5 px-6 py-4 backdrop-blur">
        <div className="flex items-center gap-4">
          <SidebarTrigger />
          <div>
            <p className="text-xs uppercase tracking-[0.25em] text-indigo-200">Notificaciones</p>
            <h1 className="text-3xl font-bold text-white">Centro de alertas</h1>
            <p className="text-sm text-slate-200">Configura qué eventos quieres recibir.</p>
          </div>
        </div>
      </header>
          ...
          <Card className="border-white/10 bg-white/5 text-white shadow-xl backdrop-blur">
            ...
            {notificaciones.length === 0 ? (
              <div className="flex flex-col items-center justify-center gap-3 rounded-lg border border-dashed border-white/20 bg-white/5 px-6 py-8 text-center">
                ...
                <p className="text-sm text-slate-200">Sin notificaciones. Llegarán aquí cuando se activen.</p>
                <Badge variant="secondary" className="bg-indigo-500/20 text-indigo-100 border border-white/20">
                  Centro limpio
                </Badge>
              </div>
            ) : (
              notificaciones.map((n) => (
                <div key={n.id} className="rounded-lg border border-white/15 bg-white/10 p-4 shadow-sm backdrop-blur">
